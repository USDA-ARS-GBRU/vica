...
time sketchblacklist.sh -Xmx63g in=$datadir/sorted.fa.gz prepasses=1 tree=auto taxa taxlevel=species ow out=blacklist_refseq_species_300.sketch mincount=300 k=19,24

#Generate 31 sketch files, with one sketch per species.
time bbsketch.sh -Xmx63g in=$datadir/sorted.fa.gz out=taxa#.sketch mode=taxa tree=auto accession=null gi=null files=31 ow unpigz minsize=400 prefilter autosize blacklist=blacklist_refseq_species_300.sketch k=19,24
